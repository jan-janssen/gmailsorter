...
from mailsort.imap.authentication import create_service
from mailsort.imap.mail import ImapMailBase
...
class ReconnectingImapMailBase(ImapMailBase):
    ...
    This stands in for mailsort.local.Imap, which is the class that actually knows
...
class TestImapAuthentication(TestCase):
    @patch("mailsort.imap.authentication.IMAP4_SSL")
    ...
    @patch("mailsort.imap.authentication.IMAP4")
...
class TestImapMailBase(TestCase):
    ...
    @patch("mailsort.imap.mail.get_email_dict")
...
class TestImapLocalHelpers(TestCase):
    @patch("mailsort.local.ImapMailBase.__init__", return_value=None)
    @patch("mailsort.local.create_imap_service")
    @patch("mailsort.local.Imap._create_databases")
